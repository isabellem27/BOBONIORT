      ...
      *    gestion des erreurs de saisie
       ...
       01  WS-MESSAGE.
           05 WS-MESSAGE11               PIC X(31)
               VALUE 'ERREUR DE SAISIE, VEUILLEZ VALI'           .
           05 WS-MESSAGE12               PIC X(31)
               VALUE 'DER VOTRE CHOIX EN PREMIER.'               .
       01  WS-MESSAGE2.
           05 WS-MESSAGE21               PIC X(31)
               VALUE 'VEUILLEZ SELECTIONNER UNE VALEU'           .
           05 WS-MESSAGE22               PIC X(31)
               VALUE 'R POUR CHAQUE PRESTATION.'                 .  
       01  WS-MESSAGE3.
           05 WS-MESSAGE31               PIC X(31)
               VALUE 'VEUILLEZ NE SELECTIONNER QUE UN'           .
           05 WS-MESSAGE32               PIC X(31)
               VALUE 'E VALEUR POUR CHAQUE PRESTATION'           . 
      ...
      ******************************************************************
       SCREEN SECTION.
           COPY 'SPECIFIC-CONTRACT-SCREEN.cpy'.   
      ...
      ****************************************************************** 
       1000-SCREEN-LOOP-START.  
           ...
           MOVE 195063475290876 TO LK-CUS-CODE-SECU .
           SET LK-CUS-NBCHILDREN TO 3.
           MOVE '1995-09-27' TO LK-CUS-BIRTH-DATE. 

           PERFORM 1100-PREPARE-SCREEN-START 
                    THRU END-1100-PREPARE-SCREEN.     
           PERFORM UNTIL WS-SELECT-OPTION = 'TRUE'            
              ACCEPT SPECIFIC-CONTRACT-SCREEN  
      ...
      ******************************************************************
       ...
       1450-PREPARE-CRS-COUT-START.
           EXEC SQL 
              DECLARE CRSCOUT CURSOR FOR
                 ...
                 AND cr.CLASSIC_REIMBURSEMENT_LABEL = 'ALLEGE'                  
           END-EXEC.
           IF  SQLCODE NOT = ZERO 
       ...
       1500-CRS-CLASSIC-READ-START.
           EXEC SQL 
              FETCH CRSCLASSIC 
              INTO 
              :SQL-CLAS-LABEL, :SQL-CLAS-DOCTOR,
      ...
      *          alors j'enregistre dans la table.
                             ...
                             PERFORM 6100-SQL-INSERT-START
                                      THRU END-6100-SQL-INSERT                                 EXEC SQL COMMIT WORK END-EXEC         
                             PERFORM 1300-SQL-DISCONNECTION-START
                                   THRU END-1300-SQL-DISCONNECTION 
                             CALL 'menucont' USING CONTENT LK-CUSTOMER          
                          END-IF
      ...
      ****************************************************************** 
       ...
       9210-ERROR-MESSAGE-START.
           MOVE WS-MESSAGE2 TO SC-MESSAGE. 
           DISPLAY SC-MESSAGE
           LINE 32 COL 20 FOREGROUND-COLOR IS 7. 
           PERFORM 8000-INITIALIZE-SCREEN-START
              THRU END-8000-INITIALIZE-SCREEN.                                            
       ...
       9220-ERROR-MESSAGE-START.
           MOVE WS-MESSAGE3 TO SC-MESSAGE. 
           DISPLAY SC-MESSAGE
           LINE 32 COL 20 FOREGROUND-COLOR IS 7. 
           PERFORM 8000-INITIALIZE-SCREEN-START
              THRU END-8000-INITIALIZE-SCREEN.                                            
