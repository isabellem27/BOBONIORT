       ...
       2220-SQL-CRS-AGE-REPART-START.
          EXEC SQL 
              DECLARE CRSAGE CURSOR FOR
                 SELECT CUSTOMER_GENDER, 
                       FLOOR(EXTRACT(YEAR FROM AGE(CUSTOMER_BIRTH_DATE))
                        / 10) * 10 AS AGE,
                       COUNT(UUID_CUSTOMER) AS NB_CUSTOMER
                 FROM CUSTOMER 
                 WHERE CUSTOMER_ACTIVE  IN ('0', '1')
                 GROUP BY CUSTOMER_GENDER, AGE
                 ORDER BY CUSTOMER_GENDER DESC, AGE ASC   
           END-EXEC.
           IF  SQLCODE NOT = ZERO 
      ...
      *     END-PERFORM. 
       ...
       2320-CRS-AGE-READ-START.
           EXEC SQL 
              FETCH CRSAGE
              INTO :SQL-AGE-GENRE, :SQL-AGE-AGE, :SQL-AGE-NB-CUST    
           END-EXEC.
           IF  (SQLCODE NOT = ZERO) AND (SQLCODE NOT = FIN) THEN
